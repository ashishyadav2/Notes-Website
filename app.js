    ...
    MultipleUser.findOne({ username: userProfileId }, (err, foundNotes) => {
      if (!err) {
        ...
        if (foundNotes.notesList.length === 0) {
          res.render("home", {
            noteListArray: foundNotes.notesList,
            userProfileId: foundNotes.username,
            text: text,
          });
        } else {
          text = "";
...
app.post("/profile/:profileId/post", (req, res) => {
  ...
  MultipleUser.findOne({ username: userName }, (err, foundNotes) => {
    if (!err) {
      MultipleUser.findOne(
        { notesList: { $elemMatch: { _id: noteId } } },
        (err, foundNote) => {
          if (noteId === "") {
            if (!foundNote) {
            ...
            MultipleUser.updateOne(
              { "notesList._id": noteId },
              { "notesList.$.title": title, "notesList.$.content": content },
              (err) => {
                if (!err) {
                  res.redirect(`/profile/${userName}`);
                }
...
app.post("/profile/:profileId/edit", (req, res) => {
  ...
  const userName = req.params.profileId;
  MultipleUser.findOne(
    { notesList: { $elemMatch: { _id: editId } } },
    { notesList: { $elemMatch: { _id: editId } } },
    (err, result) => {
      if (!err) {
        if (result) {
          res.render("createNote", {
            noteInfo: result.notesList[0],
...
app.post("/profile/:profileId/delete", (req, res) => {
  ...
  const username = req.params.profileId;
  MultipleUser.findOneAndUpdate(
    { username: username },
    { $pull: { notesList: { _id: delId } } },
    (err, foundList) => {
      if (!err) {
        res.redirect(`/profile/${username}`);
      }
...
app.post("/login", (req, res) => {
  ...
  req.login(newUser, (err) => {
    if (err) {
      console.log(err);
      errMessage =
        '<div><h1>Invalid id or password. <a href="/login">Try again</a></h1></div>';
      res.send(errMessage);
    } else {
...
app.post("/register", (req, res) => {
  ...
  MultipleUser.register({ username: username }, password, (err, user) => {
    if (err) {
      console.log(err);
      errMessage =
        '<div><h1>Username already exists. <a href="/register">Try again</a></h1></div>';
      res.send(errMessage);
    } else {
      passport.authenticate("local")(req, res, () => {
        res.redirect(`/profile/${username}`);
